...
//    private final IUserDAO userDAO = new UserDAOImpl();    //todo
//    private final IUserService userService = new UserServiceImpl(userDAO); //todo
...
//        UserLoginDTO userLoginDTO = new UserLoginDTO(username, password); //todo

        boolean principleIsAuthenticated = false;
//        try {
//            principleIsAuthenticated = AuthenticationProvider.authenticate(userLoginDTO); //todo
//
...
//            session.setAttribute("role", userService.getUserByUsername(username).getRoleType().name()); //todo
//
...
//
//        } catch (UserDAOException | UserNotFoundException e) { //todo
//            //response.sendRedirect(request.getContextPath() + "/login?isError=true");
